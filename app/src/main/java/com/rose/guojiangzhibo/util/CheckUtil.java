...
public class CheckUtil {
	 ...
	 * @return
	 */
	public static boolean isOrganizationCode(String value){
		String regExp = "^\\d{8}[X|0-9]$";
		Pattern p = Pattern.compile(regExp);
		Matcher m = p.matcher(value);
		return m.find();
	}
	/**
	 * 社会信用代码，18位阿拉伯数字或英文字母
	 * @param value
	 * @return
	 */
	public static boolean isCommunityCreditCode(String value){
		String regExp = "^[0-9a-zA-Z]{18}$";
		Pattern p = Pattern.compile(regExp);
		Matcher m = p.matcher(value);
		return m.find();
	}
	/**
	 * 军人证，为汉字、阿拉伯数字和英文字符的组合，必须包含汉字和阿拉伯数字，不超过11位，不少于6位。首位必须为汉字。
	 * @param value
	 * @return
	 */
	public static boolean isSoldierCard(String value){
		String regExp = "^[\\u4e00-\\u9fa5][A-Za-z0-9\\u4e00-\\u9fa5]{5,10}$";
		Pattern p = Pattern.compile(regExp);
		Matcher m = p.matcher(value);
		return m.find();
	}
	/**
	 * 工商注册号码，总计15位，仅限使用阿拉伯数字
	 * @param value
	 * @return
	 */
	public static boolean isBusinessRegistNo(String value){
